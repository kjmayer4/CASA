...
% Last edited 2/26/2022 KJM
% Plots time series of SMPS sizing data as total number, total mass, and
% a size distribution

%% Import data
%Define path where data is located
path = 'C:\Data\CASA\SMPS_data\Exported_data';
%Import to matlab as a cell arry (row 1 is time/date, row 2 is diameter, 
%row 3 is dN/dlogDp
smps = importSMPS(path);

%Pull date in first and last sample for naming
startdate = datestr(smps{1,1});
startdate = startdate(1:11);

%Return to folder to save figures/analysis products
cd('C:\Data\CASA\SMPS_data\Quicklooks')

%% Process data
%Sort date
smps = SortSizingDate(smps);
...
% Print and save
print(gcf,['CASA_smps_quicklook_', startdate,'.png'], '-dpng','-r300') %exports figure to a .png file
